...
const MODEL_COSTS: Record<ModelTier, number> = {
  ...
  standard: 0.2, // $0.00020 per 1K
  heavy: 0.8, // $0.00080 per 1K
};
...
export function getAgentModelConfig(
  ...
  } else if (agentType === "functional") {
    // Functional agents 使用默认配置
...
export function generateModelReport(config?: HiveConfig): string {
  ...
  for (const [tier, model] of Object.entries(models.tierMapping)) {
    if (model) {
      ...
      lines.push(
        `  - ${tier.padEnd(10)} → ${model.padEnd(20)} | Cost: $${(cost * 0.00001).toFixed(4)}/1K | Latency: ${latency}ms/1K`,
      );
    }
  }
  lines.push("");
  ...
  for (const [agent] of Object.entries(models.system)) {
    const modelUsage = getAgentModelConfig(agent as "orchestrator", undefined, config);
    lines.push(
      `  - ${agent.padEnd(12)} → tier=${modelUsage.tier}, temp=${modelUsage.temperature}, max=${modelUsage.maxTokens}`,
    );
  }
  lines.push("");
  ...
  lines.push(
    `  - tier=${defaultModel.tier}, temp=${defaultModel.temperature}, max=${defaultModel.maxTokens}`,
  );
  lines.push("");
  ...
  if (Object.keys(models.functional.overrides).length > 0) {
    ...
    for (const [role] of Object.entries(models.functional.overrides)) {
      const modelUsage = getAgentModelConfig("functional", role, config);
      lines.push(
        `  - ${role.padEnd(25)} → tier=${modelUsage.tier}, temp=${modelUsage.temperature}, max=${modelUsage.maxTokens}`,
      );
    }
    lines.push("");
  }
